...
void main (void) 
{
	...
	vec3 color3 = vec3(0.6, 0.6, 0.1); 
	vec3 color4 = vec3(0.5, 0.5, 0.2); 
	
	vec3 mix1 = mix(color1, color2, snoise(interpolatedPosition * 5 + 4 ));
	
	vec3 mix2 = mix(color4, color3, snoise(interpolatedPosition * 6 + 10));
	
	vec3 finalColor = mix(mix1, mix2, step(snoise(interpolatedPosition * 6), 0.01));
	
	vec3 materialAndLighting = finalColor * totalDiffuse + totalSpecular; 
